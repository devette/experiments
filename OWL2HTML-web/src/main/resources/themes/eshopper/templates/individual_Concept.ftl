...
<@c.page title="${individual.getLocalLabel(context.locale)}" ontology=ontology>
	...
	<#macro recurse_macro objectproperty depth maxdepth propertyfilter>
	  <#if (depth?number < maxdepth?number) >
		  <li>
			  <#if (depth?number > 0) >
					 <#list 1..depth as i>&nbsp;&nbsp;</#list> 
			  </#if>
			  <a href="select?objectproperty=${objectproperty.iri?url}"><nobr><i class="fa fa-long-arrow-right"></i> ${objectproperty.getLocalLabel(context.locale)}</nobr></a>&nbsp;&nbsp;
			  <a href="select?individual=${objectproperty.object.iri?url}"><nobr><i class="fa fa-circle-o"></i> ${objectproperty.object.getLocalLabel(context.locale)}</nobr></a>&nbsp;&nbsp;
			  <#list objectproperty.object.classes as clazz><a href="select?class=${clazz.iri?url}"><i class="fa fa-bullseye"></i> ${clazz.getLocalLabel(context.locale)}</a> </#list>
		  
		  <#list objectproperty.object.objectProperties as childProperty>
		 	<#if childProperty.propertyLabel == propertyfilter>
		  		<@recurse_macro objectproperty=childProperty depth=depth+1 maxdepth=maxdepth propertyfilter=propertyfilter/>
		  	</#if>
		  </#list>
		  </li>
	  </#if>
	...
	<h2 class="title text-center"><nobr><i class="fa fa-circle-o"></i> CONCEPT: ${individual.getLocalLabel(context.locale)}</nobr></h2>
				...
				<div class="panel-body">
					<table class="table">
						<#list individual.objectProperties as p>
							<tr>
								<td><ul><@recurse_macro objectproperty=p depth=0 maxdepth=10 propertyfilter="narrower"/></ul></td>
