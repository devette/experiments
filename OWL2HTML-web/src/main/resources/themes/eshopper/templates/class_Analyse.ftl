						...
						<div>
							<p>
								...
								<#list individual.objectProperties as p>
									<tr>
										<td><ul><@recurse_macro objectproperty=p depth=0 maxdepth=10 propertyfilter="waarom"/></ul></td>                  
					                    <td style="vertical-align:top"><#list p.object.classes as clazz><a href="select?class=${clazz.iri?url}"><i class="fa fa-bullseye"></i> ${clazz.getLocalLabel(context.locale)}</a> </#list></td>
					                </tr>
								</#list>
								</table>
							</p>
